...
contract supplyChain {
    ...
    event TransferOwnership(uint32 indexed productId); // indexed keyword allow this field to be indexed on blockchain for this event
    ...
    function newOwner(uint32 _user1Id,uint32 _user2Id, uint32 _prodId) onlyOwner(_prodId) public returns (bool) {
        participant memory p1 = participants[_user1Id]; // memory will not be interacting with blockchain
